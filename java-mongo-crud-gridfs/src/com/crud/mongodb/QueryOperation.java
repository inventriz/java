...
public class QueryOperation {
	...
	public static void main(String[] args) {
		...
		try {
			...
			DBCollection collection = db.getCollection("eduquery");
...
System.out.println("\n1. Get 'name' field only");
			...
			DBCursor cursor2 = collection.find(allQuery, fields); // db.mycoll.find({}, {name : 1})
			...
			list.add(4);
			list.add(5);
			inQuery.put("number", new BasicDBObject("$in", list)); // {number : {$in : [2,4,5]}}
			...
			neQuery.put("number", new BasicDBObject("$ne", 4)); // {number : {$ne : 4}}
			...
			System.out
					.println("\n3. Find when number = 2 and name = 'rana-2' example");
			BasicDBObject andQuery = new BasicDBObject();

			List<BasicDBObject> obj = new ArrayList<BasicDBObject>();
			obj.add(new BasicDBObject("number", 2));
			obj.add(new BasicDBObject("name", "rana-2"));
			...
			System.out
					.println("\n4. Find where name = 'rana.*-[1-3]', case sensitive example");
			BasicDBObject regexQuery = new BasicDBObject();
			regexQuery.put("name", new BasicDBObject("$regex", "rana.*-[1-3]")
					.append("$options", "i"));
